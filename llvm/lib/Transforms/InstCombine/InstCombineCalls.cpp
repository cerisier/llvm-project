...
Instruction *InstCombinerImpl::visitCallInst(CallInst &CI) {
  ...
  case Intrinsic::vector_reduce_and: {
    ...
    // For last the iN bitcast will fail as the vector and value
    // generated are no the same. Until KVX-codegen-prepare can't
    // handle all possible variations generated by multiple runs of
    // inst-combine, it is simpler to just treat the bit-vector
    // reductions.
    if (match(Arg, m_ZExtOrSExtOrSelf(m_Value(Vect))) && false) {
